...
- **Impeccable critique all-in — harden/layout/clarify/adapt/polish** — `PomodoroContainer.tsx:55-653` refactor integral (5 P de la crítica 28/40): **harden**: `R`/`Reset` ya no destruye sin confirmar — nuevo `requestReset`/`ConfirmModal` con `isDirty` (`formatTime(timeLeft) left in ${phase} · Session X/Y will be lost`) y `Toast` con `Undo` vía `restoreSnapshot` en `usePomodoro.ts:285-305` (restaura `timeLeftFocus/Break`, `currentSession/Phase`, `isRunning` + `endTime`); `ModeSelector.tsx:16-48` nunca bloquea click cuando `isRunning` — muestra confirm `"This will pause the timer. Your progress (...) will be reset"` + hint inline `id="mode-hint" Pauses to switch modes` y `aria-describedby` (antes `disabled opacity-50` + `title` invisible en touch/screen-reader); `TimerControls.tsx:34,48,59` añade `aria-keyshortcuts="Space/T/R"` y `title` con atajo; `R`/`T` en `PomodoroContainer.tsx:298` y PiP `364` ahora respeta `activeMode==="flex"` y `requestReset`. **layout**: header `w-[26px]→w-8 h-8 rounded-lg gap-1→gap-1.5` + `focus-visible:ring-offset-surface` para 32px visual / 44px hit área (`PomodoroContainer.tsx:394-456`); perfil demovido de pill dominante `bg-white/[0.04] border-line-soft` a link faint `bg-white/[0.02] border-transparent text-muted` `mt-1.5 mb-2` y `ModeSelector` promovido a `mt-3 mb-3` como decisión primaria; `C/F` totals ahora siempre visibles (`text-faint tabular-nums`) no `hidden sm:inline`. **clarify**: Flex `PhaseCard.tsx:74` `banked→remaining` + `rounded-xl→rounded-[10px]` + `text-4xl→text-[38px] leading-none` para paridad con ring 38px; `SessionBar.tsx:12-43` nuevo `variant="budget"` con ticks por sesión y `pct%` label + `aria-label="Budget X% used"`; `PomodoroContainer.tsx:323-350` elimina `flexSyntheticSession` (mentira `floor(progress*sessions)+1`) y usa `flexBudgetProgress = (total-remaining)/total` sobre `timeLeftFocus+timeLeftBreak` (`flexTotalMinutes*60`) + `F {duration} · {min left} min left` + microcopy `Flex · you control — switch with T`; `totalRemainingSeconds` en Flex ahora `timeLeftFocus+timeLeftBreak` (corrige sobreconteo `+ remainingSessions*(focus+break)`). **adapt**: kbd legend `PomodoroContainer.tsx:576` elevado `text-faint→text-muted` + `role="note"` + chips `bg-white/[0.08] text-ink` `px-1.5 py-1` y `flex-wrap`; header y kbd centrados; `UpdateBanner.tsx:15` y `Toast.tsx:23` `z-[70]→z-[71]/69` + `max-w-[90vw] truncate` para no desbordar en 320px. **polish**: `DESIGN.md:19-53` rampa tipográfica completa (`label 11px`, `micro 10px`, `chip 12px/13px`, `title-sm 15px`, `pip-micro 9px`, `pip-display 24px`) limpia `detect.mjs` de 69→0 hallazgos; `ProgressRing.tsx:20` `strokeWidth*2→strokeWidth+2` (radius 76→80) fiel a `reference.html` r=80; `PhaseCard.tsx:30` `rounded-xl→rounded-[10px]` banked jerarquía; `Onboarding.tsx:62` neutraliza highlight Flex `border-focus/30 bg-focus/[0.07]→border-line bg-white/[0.03]` para no sesgar; `Toast.tsx:10` soporte `actionLabel/onAction/duration` para Undo 5s. Verificación `npm run build` ✓ y `detect --json src/app/components/pomodoro` clean.
...
- **Ring: glow sin recorte y dot anclado al tip** — `ProgressRing.tsx:14-105` corregidos los dos defectos del screenshot: **glow cortado**: `viewBox` expandido con `pad=14` (`-14 -14 204×204` para `size=176`) + `overflow:visible` en wrapper y SVG y glow externo `inset:-6 blur-[18px] opacity-11 scale-0.96` para que `drop-shadow(0 0 6px)` y `blur` no se recorten en el borde del viewport; rotación del arco migrada de clase CSS `-rotate-90` (origen inconsistente en SVG) a `transform="rotate(-90 cx cy)"` nativo. **dot desfasado**: `dotX/Y` ahora compensan `strokeLinecap="round"` (`capCompDeg=(strokeWidth/2/circumference)*360` sustraído proporcional a `progress`) para que el centro del dot coincida con el borde del cap y no con su centro geométrico; umbral `showDot 0.008-0.992` evita salto en el cierre. `PipTimer.tsx:42-67` aplica `pad=10` y mismo `rotate` nativo para consistencia en PiP. `PomodoroContainer.tsx:533` asegura `isRunning` prop y `globals.css:61` `ring-breathe`.

### Added

- **Delight: contador Classic como instrumento de precisión** — `ProgressRing.tsx:13-78` (`PomodoroContainer.tsx:532` modo classic) ahora se siente como reloj de cuerda silencioso: órbita `head dot` de 7/3.5px (`dotX/dotY` polar `progress*360-90`) con `transition-[stroke-dashoffset] 500ms` + `drop-shadow` y glow `bg-current blur-[14px] opacity-10` solo cuando `isRunning`; 12 ticks instrumentales (`stroke rgba(255,255,255,0.09)`, cardinales 5px vs 2.5px) alrededor del aro; respiración `ring-breathe 4s ease-in-out` (`globals.css:60` `@keyframes` scale 1→1.012) con `motion-safe`; `isRunning` prop (`ProgressRing.tsx:13`) pasado desde `PomodoroContainer.tsx:532`; respeta `prefers-reduced-motion` y no añade coste en paused. Tesis: cada segundo es un avance mecánico medido, el final es una liberación suave — delight quiet para 100+ usos.
...
- **Flex Switch: transición pulida al alternar fase (T/Switch)** — `globals.css:68` añade `flex-active-enter` 440ms (`translateY(9px) scale(0.986)`) y `flex-banked-enter` 400ms +68ms (`translateY(-7px) scale(0.989)`) con `cubic-bezier(0.16,1,0.3,1)`, `phase-label-anim`/`phase-time-in` (`blur 2px→0`) y `flex-shimmer-sweep` acotado a la tarjeta; `PhaseCard.tsx:11` expone `animationKey` y remonta con `key` para reiniciar la orquesta sin `width/height` layout-thrashing, `transition-[border-color,background-color]` 420ms; `PomodoroContainer.tsx:138` orquesta `flexSwitchKey` (solo switches reales en Flex — ignora hidratación y cambio de modo) y lo propaga a ambas `PhaseCard` y al `kbd T` (`motion-safe:animate`); `TimerControls.tsx:15` anima icono con `switch-icon-flip` 180deg + bump 300ms vía `bumpOn`/`requestAnimationFrame` sin perder focus; `SessionBar.tsx:33` sincroniza `bg-focus↔break` con `transition-[width,background-color]` 480ms. Todo tras `prefers-reduced-motion: no-preference` con `will-change` aislado.
