...
- **PiP con aro circular y atajos (C9)**: `PipTimer` (`src/app/components/pomodoro/PipTimer.tsx:27`) ahora muestra un aro circular de 78px con progreso (`circumference`/`dashOffset`) que integra `phase`, `time` y `ends HH:MM` en su interior, reemplazando la barra horizontal para ahorrar altura vertical. Se añadió `endsAtLabel` combinado (`classic`/`flex`) y soporte de teclado dentro de la ventana PiP (`Space`/`R`/`T`/`Esc` via `pipWindow.addEventListener` en `PomodoroContainer.tsx:332`). Se eliminó el hint inferior `space·r·t` y se ajustó la ventana a `260×148` (`usePictureInPicture.ts:44`) para que el nuevo layout encaje sin recortes.
