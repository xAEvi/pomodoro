...
- **Controlar el timer desde la pantalla bloqueada (Media Session)**: nuevo toggle "Lock screen controls" en Settings (apagado por defecto) que expone el Pomodoro en los controles de medios del sistema — pantalla bloqueada, notificación persistente en Android, auriculares Bluetooth — con `play`/`pause`/`stop` conectados al timer. Como el sistema operativo solo muestra esos controles si hay audio real reproduciéndose, `useMediaSession` mantiene su propio `AudioContext` con un loop de silencio digital (generado, sin asset externo) enrutado a un `<audio>` oculto vía `MediaStream`, independiente del sonido ambiental existente — así los controles sobreviven a pausas y a breaks, no solo mientras el timer corre. Se corrigieron dos bugs reales en el camino: una condición de carrera donde pausar dentro de los primeros ~200ms de iniciar (antes del primer tick) apagaba los controles en vez de mantenerlos, y el botón "stop" del lock screen no cerraba la sesión correctamente (dejaba audio/metadata activos). Ambos verificados con Playwright interceptando la API real de Media Session.
